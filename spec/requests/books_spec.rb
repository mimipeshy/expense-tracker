...
RSpec.describe 'Books API', type: :request do
  ...
  describe 'POST /books' do
    # valid payload
    let(:valid_attributes) { { name: 'Learn React', image_url: 'img4', author: 'peris' } }

    context 'when the request is valid' do
      before { post '/books', params: valid_attributes }

      it 'creates a book' do
        expect(json['name']).to eq('Learn React')
      end
    ...
    context 'when the request is invalid' do
      ...
      it 'returns a validation failure message' do
        expect(response.body)
          .to match(/Validation failed: Name can't be blank, Author can't be blank/)
      end
    end
  end
